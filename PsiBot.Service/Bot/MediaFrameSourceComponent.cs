...
namespace PsiBot.Services.Bot
{
    ...
    public class MediaFrameSourceComponent : ISourceComponent
    {
        private readonly ConcurrentDictionary<string, 
            SymblWebSocketService> webSocketConnectionPairs;
        ...
        public MediaFrameSourceComponent(
            ref ConcurrentDictionary<string, SymblWebSocketService> webSocketConnectionPairs,
            Pipeline pipeline, 
            CallHandler callHandler, IGraphLogger logger)
        {
            ...
            this.webSocketConnectionPairs = webSocketConnectionPairs;
        ...
        public void Received(AudioMediaBuffer audioFrame)
        {
            ...
            if (audioFrame.UnmixedAudioBuffers != null)
            {
                foreach (var buffer in audioFrame.UnmixedAudioBuffers)
                {
                    ...
                    if (identity != null)
                    {
                        string participantId = identity.Id;
                        webSocketConnectionPairs[participantId]
                                .SendAudio(data);

                        buffers.Add(participantId, 
                            (new AudioBuffer(data, audioFormat), audioFrameTimestamp));
                    }
                    else
                    {
